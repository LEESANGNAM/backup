...
<%  
	...
	Connection con = null;
	String sql = "(select fname,faddress,fcount,RANK( ) OVER (ORDER BY fcount DESC, fname) AS RANK from bestfood)";
	try{
		con = DriverManager.getConnection(url, user, password); // Connection 객체 생성(db 연결) 
		ppst = con.prepareStatement("select * from"+sql+"where RANK between 91 and 100"); // PreparedStatement 객체 생성(쿼리 생성)
